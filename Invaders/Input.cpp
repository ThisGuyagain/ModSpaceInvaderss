...
void*  Input::ProcessMouseThread(void* arg)
{
	...
	if (fmouse != NULL)
	{
		while (((Input*)arg)->mQuit == false) // so as long as mQuit is FALSE, this will endlessly loop
		{
			...
			if (((Input*)arg)->TheMouse.PositionX > ((Input*)arg)->Xlimit) ((Input*)arg)->TheMouse.PositionX = ((Input*)arg)->Xlimit;
			if (((Input*)arg)->TheMouse.PositionX < 0) ((Input*)arg)->TheMouse.PositionX = 0;
			
			((Input*)arg)->TheMouse.PositionY += (mousey / 1.0f);
			
			if (((Input*)arg)->TheMouse.PositionY > ((Input*)arg)->Ylimit) ((Input*)arg)->TheMouse.PositionY = ((Input*)arg)->Ylimit;
...
int Input::AreYouMyKeyboard()
{
	...
	std::string pattern = "event-kbd"; //<-change to event-mouse if your BT keyboard is a "mouse"
	std::string file = "";
...
void Input::SetLimits(int Xlim,int Ylim)
{
	Xlimit = Xlim;
	Ylimit = Ylim;
}
